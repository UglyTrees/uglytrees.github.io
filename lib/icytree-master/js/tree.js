...
// Number the labelled nodes according to a label-id mapper.
// Number the remaining nodes anything after those numbers
Tree.prototype.sortNodeIDs = function(labelIDmap) {
    var nodeID = labelIDmap.len;
    for (var i=0; i<this.getTreeNodeList().length; i++) {
	var node = this.getTreeNodeList()[i];
	if (node.label != null) node.id = labelIDmap.map[node.label];
	else node.id = nodeID++;
    }
};
