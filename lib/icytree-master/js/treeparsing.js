...
TreeFromNewick.prototype.doParse = function(tokenList, newick) {
    ...
    function ruleN(parent) {
        var node = new TreeNode(thisTreeNodeID++);
        if (parent !== undefined)
...
function TreeFromPhyloXML (phylogenyElement) {
    ...
    function annotateTreeNode(node, prefix, elements) {
    ...
    function walkDom(parent, cladeElement) {
        ...
        for (var i=0; i<cladeElement.children.length; i++) {
            ...
            switch(tagName) {
                ...
                case "taxonomy":
                    annotateTreeNode(node, "taxonomy", childEl.children);
                    break;

                case "sequence":
                    annotateTreeNode(node, "sequence", childEl.children);
                    break;
...
// Creates a node label to id mapping so that label ids are constant across all trees
function buildLabelIDMapping(trees) {

	var labelIDmap = {map: {}, len: 0};
	var id = 0;
	for (var i = 0; i < trees.length; i ++){
		var tree = trees[i];
		for (var j = 0; j < tree.nodeList.length; j++) {
			var node = tree.nodeList[j];
			if (node.label != null && labelIDmap.map[node.label] == null) labelIDmap.map[node.label] = id++;
		}
	}
...
// If any annotations are lists, splits them into multiple annotations
function splitAnnotationLists(trees) {


	// Get a unique list of annotations
	vectorised_annotations = [];
	for (var t = 0; t < trees.length; t++) {
		var tree = trees[t];
		for (var i = 0; i < tree.nodeList.length; i ++){
			var ann = tree.nodeList[i].annotation;
			for (var a in ann) {
				var value = tree.nodeList[i].annotation[a];
				if (Array.isArray(value) && !vectorised_annotations.includes(a)) vectorised_annotations.push(a);
			}
		}
	}
	...
	for (var i = 0; i < vectorised_annotations.length; i ++) {


		var annotation_name = vectorised_annotations[i];

		// Split the annotation into 1 annotation for each element in the vector and delete the original annotation
		for (var t = 0; t < trees.length; t++) {
			var tree = trees[t];
			for (var j = 0; j < tree.nodeList.length; j ++){
				var values = tree.nodeList[j].annotation[annotation_name];
				if (!Array.isArray(values))  tree.nodeList[j].annotation[annotation_name + "[0]"] = values;
				else for (var v = 0; v < values.length; v++){
					var val = values[v];
					tree.nodeList[j].annotation[annotation_name + "[" + v + "]"] = val;
				}
				 delete tree.nodeList[j].annotation[annotation_name];
			}
...
function calculateNDescendents(node){

	if (node.children.length == 0){
		node.ndescendents = 0;
		return node.ndescendents;
	}


	node.ndescendents = 2 + calculateNDescendents(node.children[0]) + calculateNDescendents(node.children[1]);

	if (node.children[0].ndescendents > node.children[1].ndescendents) {
...
function getTreesFromString(string) {
    ...
    // Ensure that the tree is binary and that the labels are consistently identified
    var labelIDmap = buildLabelIDMapping(trees);
    for (var i = 0; i < trees.length; i ++){
	trees[i].sortNodeIDs(labelIDmap);
	for (var j = 0; j < trees[i].nodeList.length; j ++){
            var node = trees[i].nodeList[j];
	    if (node.children.length != 2 && node.children.length != 0) throw new ParseException("Tree " + i + " is not binary");
	}

	calculateNDescendents(trees[i].root);




    }
    splitAnnotationLists(trees);
